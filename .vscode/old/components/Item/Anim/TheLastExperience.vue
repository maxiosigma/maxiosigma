			...
			class Robot {
				constructor(color, light, size, x, y, struct) {
					...
					for (const link of struct.links) {
						...
						const dy = p0.y - p1.y
						this.links.push(
							new Robot.Link(this, p0, p1, Math.sqrt(dx * dx + dy * dy), (link.size * size) / 3, link.lum, link.force, link.disk)
						)
					}
				}
				update() {
					...
					if (this === pointer.dancerDrag && this.size < 16 && this.frame > 600) {
						...
						dancers.push(
							new Robot(this.color + 90, this.light * 1.25, this.size * 2, pointer.x, pointer.y - 100 * this.size * 2, struct)
						)
