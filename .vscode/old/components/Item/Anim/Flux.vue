<template>
	<div class="flux">
		<canvas class="flux-canvas"></canvas>
		<h2 class="flux-title">Press down</h2>
	</div>
</template>
...
export default {
	mounted() {
		...
		function Spline() {
			...
			for (var j = 0; j < 180; j++) {
				this.geometry.vertices.push(new THREE.Vector3((j / 180) * length * 2 - length, 0, 0))
				this.geometry.colors[j] = new THREE.Color('hsl(' + (j * 0.6 + this.color) + ',70%,70%)')
			}
		...
		function updateColor() {
			for (var i = 0; i < splines.length; i++) {
				...
				for (var j = 0; j < splines[i].geometry.vertices.length; j++) {
					splines[i].mesh.geometry.colors[j] = new THREE.Color('hsl(' + (j * 0.6 + color) + ',70%,70%)')
				}
