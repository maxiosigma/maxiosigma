<template>
  <canvas class="inset-0 z-0 fixed pointer-events-none" ref="wavehexagon"></canvas>
</template>
...
export default {
  mounted() {
    ;(() => {
      ...
      //  console.log(Math.max(innerWidth, innerHeight) / 2)

      const canvas = this.$refs.wavehexagon // document.createElement("canvas") //создаём Canvas
      ...
      window.onresize = function () {
        //обновляем размеры Canvas если меняется размер окна
      ...
      class Dot {
        ...
        draw() {
          ...
          ctx.closePath()
          //  ctx.fillStyle = `rgba(255, ${o}, ${o}, ${o}, ${s})`
          ctx.fillStyle = "rgba(" + 197 + ", " + 0 + ", " + 243 + ", " + s + ")"
          ctx.fill()
      ...
      function getDistance(x, y) {
        //      получаем расстояние от центра области просмотра до точки ...
