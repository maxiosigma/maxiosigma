...
Vue.mixin({
  methods: {
    ...
    menuRedirect(link) {
      ...
      setTimeout(
        () => (location.href = "/" + this.$i18n.locale + "/" + link),
        500
      );
    },
    headSplitTitle(slug, delimetr = " | ") {
      return slug != undefined
        ? String(slug).toUpperCase().split("_").join(delimetr)
        : "Link";
    },
      ...
      setTimeout(() => {
        ...
        const toast = (count) => {
          setTimeout(
            () =>
              this.$toast.show(
                "На сайте используются cookie " + (count - 1) + " из 2",
                {
          ...
          setTimeout(
            () =>
              this.$toast.show(
                "На сайте используются cookie " + count + " из 2",
                {
                  theme: this.isLight ? "outline" : "bubble",
      ...
      try {
        ...
        !(function () {
          ...
          (t.type = "text/javascript"),
            ...
            (t.src = "https://vk.com/js/api/openapi.js?169"),
            (t.onload = function () {
              VK.Retargeting.Init("VK-RTRG-1455228-5lkj2"),
                VK.Retargeting.Hit();
            }),
            document.head.appendChild(t);
        })();
